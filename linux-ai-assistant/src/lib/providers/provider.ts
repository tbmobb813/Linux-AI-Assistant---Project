...
import { getInvoke as getTauriInvoke, getListen as getTauriListen } from '../tauri-shim';

export type ProviderMessage = { role: 'user' | 'assistant' | 'system'; content: string } & Partial<Message>;
...
export function getProvider(): Provider {
    try {
        ...
        const openaiEnabled = (typeof import.meta !== 'undefined' && (import.meta as any).env && (import.meta as any).env.VITE_OPENAI_ENABLED) === 'true';
                ...
                async generateResponse(conversationId: string, messages, onChunk?) {
                    try {
                        ...
                        if (onChunk && invokeFn && listenFn) {
                            const sessionId: string = await invokeFn('provider_openai_stream', { conversation_id: conversationId, messages, model: (import.meta as any).env?.VITE_OPENAI_MODEL || null });
                            ...
                            const unlistenEndP = listenFn('provider-stream-end', (e: any) => {
                                const payload: any = e.payload;
                                if (payload?.session_id === sessionId) {
                                    Promise.all([unlistenChunkP, unlistenEndP]).then((fns: any[]) => fns.forEach((fn) => fn && fn()));
                                }
                            });
                        ...
                        const resp = await invokeFn('provider_openai_generate', { conversation_id: conversationId, messages, model: (import.meta as any).env?.VITE_OPENAI_MODEL || null });
                        return resp;
                    } catch (e: any) {
