...
test('retryMessage retries a failed user message and persists assistant response', async () => {
    ...
    const createMock = vi.fn()
        .mockImplementationOnce(async () => { throw new Error('DB write failed') })
        .mockImplementationOnce(async (data: any) => ({ id: 'm1', ...data, timestamp: Date.now() }))
        .mockImplementationOnce(async (data: any) => ({ id: 'm2', ...data, timestamp: Date.now() }));
