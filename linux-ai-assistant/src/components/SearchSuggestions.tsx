...
export const SearchSuggestions: React.FC<SearchSuggestionsProps> = ({
  ...
  useEffect(() => {
    generateSuggestions();
  }, [query, recentSearches, availableTags, recentConversations]);
  ...
  const addRecentSearchSuggestions = (suggestions: SearchSuggestion[]) => {
  ...
  const addPopularTagSuggestions = (suggestions: SearchSuggestion[]) => {
  ...
  const addRecentConversationSuggestions = (
  ...
  const addSearchTemplates = (suggestions: SearchSuggestion[]) => {
    const templates = [
  ...
  const addMatchingConversations = (
    suggestions: SearchSuggestion[],
    query: string,
  ) => {
  ...
  const addMatchingTags = (suggestions: SearchSuggestion[], query: string) => {
  ...
  const addSearchCommands = (
    ...
    const matching = commands.filter(
      (cmd) => cmd.cmd.startsWith(query) || query.startsWith(cmd.cmd),
    );
  ...
  const addFilterSuggestions = (
    ...
    const matching = filters.filter(
      (filter) => filter.text.includes(query) && filter.text !== query,
    );
