...
export default function ConversationItem({ conversation, selected, onSelect }: Props) {
    ...
    const handleRename = async (e?: FormEvent) => {
        ...
        try {
            ...
            addToast({ message: `Rename failed: ${String(err)}`, type: 'error', ttl: 5000 });
        }
    };
                ...
                action: {
                    ...
                    onClick: async () => {
                        try {
                            ...
                            useUiStore.getState().addToast({ message: 'Conversation restored', type: 'success', ttl: 3000 });
                        } catch (e: any) {
                            useUiStore.getState().addToast({ message: `Restore failed: ${String(e)}`, type: 'error', ttl: 4000 });
                        }
                    },
                },
            });
        } catch (err: any) {
            addToast({ message: `Delete failed: ${String(err)}`, type: 'error', ttl: 5000 });
        }
    };
                ...
                {editing && (
                    <form onSubmit={handleRename} className="flex gap-2">
                        <input
                            ...
                            autoFocus
                        />
                        <button type="submit" className="text-xs px-2 py-1 bg-green-600 rounded">Save</button>
                        <button type="button" className="text-xs px-2 py-1 bg-gray-600 rounded" onClick={(e) => { e.stopPropagation(); setEditing(false); setTitle(conversation.title || 'Untitled'); }}>Cancel</button>
                    </form>
                )}
            </div>
