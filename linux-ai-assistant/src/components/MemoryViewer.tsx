...
export const MemoryViewer: React.FC = () => {
  ...
  return (
    <FadeIn>
      <div className="fixed inset-y-0 right-0 z-50 w-96 bg-[#1a1b26] border-l border-[#414868] shadow-2xl flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-[#414868]">
          <div className="flex items-center gap-2">
            <Brain className="w-5 h-5 text-[#bb9af7]" />
            <div>
        ...
        {/* Token Usage Bar */}
        {projectMemory && (
          <div className="px-4 py-3 bg-[#24283b] border-b border-[#414868]">
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="text-[#9aa5ce]">Token Usage</span>
              <span className="font-mono text-[#c0caf5]">
                {projectMemory.totalTokens} / {projectMemory.maxTokens}
              </span>
            </div>
            <div className="w-full h-2 bg-[#414868] rounded-full overflow-hidden">
              <div
            ...
            {tokenPercentage > 80 && (
              <p className="text-xs text-[#e0af68] mt-1">
        ...
        <div className="p-4 border-b border-[#414868]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-3 py-2 text-sm border border-[#414868] rounded-lg bg-[#24283b] text-[#c0caf5] placeholder-[#565f89] focus:outline-none focus:ring-2 focus:ring-[#bb9af7]"
            />
          </div>
        </div>

        {/* Actions */}
        <div className="flex gap-2 px-4 py-3 border-b border-[#414868]">
          <AnimatedButton
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredMemories.length === 0 ? (
            <div className="flex items-center justify-center h-full text-[#565f89] p-8">
...
const MemoryCard: React.FC<MemoryCardProps> = ({
  ...
  return (
    <div className="bg-[#24283b] rounded-lg border border-[#414868] overflow-hidden hover:border-[#bb9af7] transition-colors">
      {/* Header */}
      <div
      ...
      >
        <button className="mt-1 flex-shrink-0">
          {isExpanded ? (
            <ChevronDown className="w-4 h-4 text-[#9aa5ce]" />
          ) : (
            <ChevronRight className="w-4 h-4 text-[#9aa5ce]" />
          )}
        </button>
        <div className="flex-1 min-w-0">
          <p className="text-sm text-[#c0caf5] line-clamp-2">
            {memory.content}
          </p>
          <div className="flex items-center gap-2 mt-2 text-xs text-[#9aa5ce]">
        ...
        <button
          ...
          className="p-1 rounded hover:bg-[#f7768e]/20 flex-shrink-0"
          title="Delete memory"
        >
      ...
      {/* Expanded Details */}
      {isExpanded && (
        <div className="px-3 pb-3 space-y-2 border-t border-[#414868] pt-3">
          {memory.context && (
            <div className="text-xs">
              <span className="font-semibold text-[#c0caf5]">Context:</span>
              <p className="text-[#9aa5ce] mt-1">{memory.context}</p>
            </div>
          )}

          {memory.tags.length > 0 && (
            <div className="flex items-center gap-1 flex-wrap">
              <Tag className="w-3 h-3 text-[#9aa5ce]" />
              {memory.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 text-xs rounded bg-[#bb9af7]/20 text-[#bb9af7]"
                >
                  {tag}
                </span>
              ))}
          ...
          {memory.conversationId && (
            <div className="flex items-center gap-1 text-xs text-[#9aa5ce]">
